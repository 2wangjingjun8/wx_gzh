	...
	*/
    public function addtop(){
        ...
        if ($this->req->isPost()){
		    ...
		    $data=$this->req->post();
			//添加数据到数据库
			$rs=$this->db->name(strtolower($this->c))->insert($data);
			//获取添加成功id
            $insertId =$this->db->name(strtolower($this->c))->getLastInsID();
			if($insertId>0){
...
*/
    public function addson(){
     ...
     if ($this->req->isPost()){
			...
			$attr=$this->db->name('attribute')->where(array())->select();
    ...
    public function del()
    {
		...
		if(empty($sons)){
			...
			if($rs>0){
					$this->success('删除成功','index/Admin/category/index');
				}else{
					$this->error('删除失败'); 
			}	
		}else{
