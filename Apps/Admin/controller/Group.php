...
class Group extends Publics
{
		...
		$html='';
 		foreach($result AS $k=>$v){
						 $html.='<tr class="list-group-item-success">';	
							...
							$html.='<td><button class="btn btn-xs btn-primary update"  onclick="window.location=`admin/'.strtolower($this->c).'/update?id='.$v['id'].'&page='.$this->req->get('page').'`"><i class="glyphicon glyphicon-search" ></i>编辑</button>';
							if($v['namestr'] != '超级管理员'){
							$html.='<a href="admin/'.strtolower($this->c).'/del?id='.$v['id'].'" class="btn btn-xs btn-danger del"><i class="glyphicon glyphicon-home del"></i>删除</a></td>';
							}
						$html.='</tr>';			
		}
		//加载模板
    ...
    public function add()
    {
		...
		if ($this->req->isPost()){
			...
			$data['rulestr'] = implode(',',$data['rulestr']);
			// dump($data);exit;
			//添加数据到数据库
		   ...
		   $rs = $this->db->name('adminmenu')->where(array())->select();
		   // dump($rs);exit;
    ...
    public function update()
    {
		...
		if(!empty($_GET['id'])){
				...
				// $rss = $this->db->name(strtolower($this->c))->getLastSql();
				// dump($rss);exit;
				if($result>0){
